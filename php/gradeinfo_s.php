<?php
	...
	$table_info .= "<p>Date Taken: " . $grade["date"] . "</p>";
	...
	foreach ($answer_array as $qID=>$s_answer) {
		...
		$table_info .= "<p>" . $q_info['question'] . "</p><p>Points: " . $question_crs['grade'] . "/" . $points_arr[$qID] . "</p>";			
		$table_info .= "<p>You wrote:</p><textarea disabled rows='10' cols='100'>" . $s_answer ."</textarea>";
		$table_info .= "<p>Code output:</p><textarea disabled rows='10' cols='100'>" . $question_crs['output'] ."</textarea>"; 
		$table_info .= "<p>Comments for question " . $count_q . ":</p> <textarea disabled name=\"comments\" id=\"" . $qID . "\" rows=5 cols=100>" . $question_crs['comment'] . "</textarea>";
	}
	
?>
<h3>Grade Information</h3>
<div id="answers"><?php echo $table_info; ?></div>
<div><button onclick="showallGrades();">Return to Grades</button></div>
